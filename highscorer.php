                ...
                <tr>
                    <th>
                        <p style="align-content: center;">
                            <h1 style="color:">Highest Scorer</h1>
                        </p>
                    </th>
                </tr>
                <tr>
                    ...
                    <table align="center" width="100%">
                            ...
                            $query = "SELECT playername,runs,team.name,rnk FROM team , (SELECT playername,runs,player.team_id, @rank := CASE WHEN @rankval = runs THEN @rank WHEN (@rankval := runs) IS NOT NULL THEN @rank + 1 END AS rnk FROM player, (SELECT @rank := 0, @rankval :=NULL) as x ORDER BY runs DESC) as p1 WHERE p1.team_id=team.team_id";                            $result = mysqli_query($con, $query);
                            [$i] = floor(1);
